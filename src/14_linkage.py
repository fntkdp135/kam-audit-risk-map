...
**왜 같은 해로 보면 안 되는가**
회계감사기준서 1701(ISA 701) 문단 15는, 의견변형을 초래하는 사항은 그 특성상 핵심감사사항
이지만 **핵심감사사항 단락에 기술해서는 안 된다**고 정한다(의견근거 단락에 기술하고, 핵심감사
사항 단락에서는 그 사항을 언급만 함). 따라서 **같은 보고서에서 두 기재가 겹치는 일은 제도상
발생하지 않으며**, 겹침을 세는 것은 발견이 아니라 기준서를 다시 읽는 것에 불과하다.

그래서 검증은 **연도를 건너뛰어** 해야 한다: t년 핵심감사사항 → 이후 연도 의견변형.
그리고 단순히 "이어졌는가"가 아니라 **"같은 사유로 이어졌는가"**를 봐야 한다.

**계속기업 관련 중요한 불확실성을 이 분석에서 제외한 이유**
초안에서는 계속기업 불확실성도 경로에 포함했으나(KAM→계속기업, 계속기업→의견변형) 기각했다.
계속기업 불확실성은 감사인의 판단이 매개하는 사건이라기보다, 회사의 존속 상태가 나쁘다는
근본 원인이 매년 다른 단락에 반복 관찰되는 것에 가깝다(ISA 570상 트리거의 상당 부분이
자본잠식·부채초과 등 재무 상태에 연동됨). 화살표로 그리면 인과처럼 보이지만 실제로는
같은 원인의 반복 관측이라, 인과 방향과 성격을 설명하려 할 때마다 기준서 해석이 더 필요해졌다.
KAM→의견변형은 이 문제가 없다: 감사인이 특정 회계쟁점을 위험으로 지목했고, 그것이 실제로
증거를 확보하지 못한 사유가 되었는가는 감사인 판단의 연속성만으로 설명되는 단일 질문이다.
"""
import os, sys, json
import pandas as pd
...
def main():
    ...
    n_mod = len(b)
    print(f"모집단: 의견변형 {n_mod}건 · 핵심감사사항 {len(k)}개 / {k.corp_code.nunique()}개사\n")
    s = d[d.경로 == "KAM → 의견변형"]
    m = int(s.일치.sum())
    print(f"■ KAM → 의견변형\n   연결 {len(s)}/{n_mod}건 ({len(s)/n_mod*100:.0f}%) · "
          f"{s.기업명.nunique()}개사 · 간격 중앙값 {s.간격.median():.0f}년\n"
          f"   그중 사유까지 일치 {m}건 ({m/len(s)*100:.0f}%) · "
          f"{s[s.일치 == 1].기업명.nunique()}개사\n")

    print("=== 사유가 일치한 사례")
    for r in d[d.일치 == 1].itertuples():
        print(f"  {r.기업명} FY{r.선행연도} {r.선행기재} → FY{r.후행연도} {r.후행기재} "
              f"(간격 {r.간격}년)")
