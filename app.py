...
# ─────────────────────────────────────────────── 02
    ...
    st.markdown('<div class="panel"><span class="h">수익인식은 오히려 안전 신호</span>'
                '수익인식이 핵심감사사항인 firm-year의 후속 발생률은 <b>0.8%</b>로, 미보유군(10.8%)보다 '
                '뚜렷하게 낮음(p=0.0002). 기업 단위로 다시 재도 3.6% vs 28.6%(p=0.002)로 유지됨. 정상 영업 중인 기업만 수익인식이 '
                '핵심위험이 되고, 존속이 흔들리는 기업은 쟁점이 계속기업·자산손상으로 옮겨가기 때문임. '
                '<b>핵심감사사항 유형은 위험의 예측이라기보다 기업의 현재 상태를 반영함.</b><br><br>'
                '특수관계자거래는 방향이 반대(23.1% vs 5.7%, p=0.043)이나 해당 기업이 6개사뿐이라 '
...
# ─────────────────────────────────────────────── 04
    ...
    steps = pd.DataFrame([
        ...
        ("④ 규칙 분류", "13개 유형, 키워드 규칙", "94.4%"),
        ("⑤ LLM 보조", "규칙 미분류분만. 판정을 코드에 고정해 재현 가능화", "3.9%"),
        ("⑥ 사람 표본검증", "무작위 60건 blind 재분류", "일치율 94.7% · κ=0.935"),
    ...
    st.markdown(f'<div class="panel"><span class="h">규칙과 LLM의 역할을 나눈 이유</span>'
                ...
                f'다시 규칙으로 옮겨 처리율을 92.8%→94.4%로 올림.</div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="panel">'
                ...
                f'· <b>κ 표본 60건.</b> 신뢰구간이 넓으며, 추출기 수정 후 재측정은 사람 분류를 다시 '
                f'받아야 해 수행하지 않음(표본 60건 중 50건 존속·분류 변경 0건·존속분 κ=0.924)<br>'
                f'· <b>항목명 추출 실패 11건</b>이 남아 있어 화면에서는 대체 문구로 표시함<br>'
