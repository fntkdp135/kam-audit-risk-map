...
# ─────────────────────────────────────────────── 02
    ...
    st.markdown('<div class="panel"><span class="h">수익인식은 오히려 안전 신호</span>'
                ...
                '뚜렷하게 낮음(p=0.003). 기업 단위로 다시 재도 5.1% vs 28.6%(p=0.004)로 유지됨. 정상 영업 중인 기업만 수익인식이 '
                '핵심위험이 되고, 존속이 흔들리는 기업은 쟁점이 계속기업·자산손상으로 옮겨가기 때문임. '
                '<b>핵심감사사항 유형은 위험의 예측이라기보다 기업의 현재 상태를 반영함.</b><br><br>'
                '특수관계자거래는 방향이 반대(23.1% vs 5.6%, p=0.040)이나 해당 기업이 6개사뿐이라 '
...
# ─────────────────────────────────────────────── 04
    ...
    st.markdown(f'<div class="panel"><span class="h">사람 검증은 정확도 확인이면서 결함 발견이었다</span>'
                f'자동 분류 결과를 감춘 채 사람이 다시 분류해 대조함. 모집단 전 구간을 덮도록 <b>FY2019~2024에서 60건, FY2025에서 15건</b>을 각각 무작위 추출했고, 유효 55건 기준 <b>일치율 94.5% · κ=0.930</b>임(구간별로는 FY2019~2024 κ=0.911, FY2025 κ=1.000). 분류 품질을 확인했고, 동시에 <b>자동화가 조용히 만든 오류</b>를 찾아냄. '
                ...
                f'고쳐 섹션 검출과 항목명 품질을 회복시킴.<br><br>'
                f'파이프라인을 세 차례 정리하는 동안(태그 병합·연결 우선·FY2025 추가) 원표본 60건 중 '
                f'<b>43건이 존속했고 분류가 바뀐 건은 0건</b>이었음. 사람이 <b>판단 불가</b>로 표시한 '
                f'3건은 모두 <b>선정 이유 구간이 비어 있던 항목</b>이라, 분류 기준의 모호함이 아니라 '
                f'추출 한계에서 나온 것이었음.</div>',
                unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="panel"><span class="h">규칙과 LLM의 역할을 나눈 이유</span>'
                ...
                f'다시 규칙으로 옮겨 처리율을 올렸음(최종 93.8%).</div>', unsafe_allow_html=True)
